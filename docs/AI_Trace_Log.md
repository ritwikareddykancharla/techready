...
* **✏️ What You Changed:** Verified the OpenRouter API payload structure, ensured Environment Variables were correctly loaded using `process.env`, and decoupled `callOpenRouter` into a reusable `utils.ts` file.
...
* **✏️ What You Changed:** Decided to avoid Tailwind CSS and keep the custom vanilla CSS glassmorphism implementation intact for a unique look.
...
* **✏️ What You Changed:** Added explicit exclusions for both `chatbot` and `legacy` folders to ensure complete isolation.
